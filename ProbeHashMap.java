    ...
    private int findSlot(int h, K k) {
        ...
        do {
            ...
            if (isAvailable(j)) {// either empty or defunct                   
                if (avail == -1) avail = j;
                if (table[j] == null) break;// first available slot
            }

            else if (table[j].getKey().equals(k)) return j;

	    j = (j+1) % capacity;// probes at nezt consecutive slot
	    
        } while (j != h);

	if (count >= maxProbes) {// if probe count is greater than current max number of probe
            maxProbes = count;
        }
